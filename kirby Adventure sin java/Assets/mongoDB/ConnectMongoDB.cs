...
public class ConnectMongoDB : MonoBehaviour
{
    ...
    private void UpdateAbsorbCounterInMongoDB()
    {
        try
        {
            ...
            var update = Builders<BsonDocument>.Update
                ...
                .Set("score", hud != null ? hud.score : 0); // Añadir el puntaje al documento

            usersCollection.UpdateOne(filter, update, new UpdateOptions { IsUpsert = true });

            Debug.Log($"Datos actualizados en MongoDB: Vida = {playerData.vida}, Enemigos absorbidos = {absorbCounter}, Tiempo de juego = {FormatearTiempoDeJuego(tiempoDeJuego)}, Puntaje = {hud?.score ?? 0}");
        }
        catch (Exception e)
        {
    ...
    public void enviarDatos()
    {
        ...
        try
        {
            ...
            Debug.Log($"Datos enviados a MongoDB: Vida = {playerData.vida}, Enemigos absorbidos = {absorbCounter}, Tiempo de juego = {FormatearTiempoDeJuego(tiempoDeJuego)}, Puntaje = {hud?.score ?? 0}");
        }
        catch (System.Exception e)
        {
